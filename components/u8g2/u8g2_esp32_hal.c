...
uint8_t u8g2_esp32_i2c_byte_cb(u8x8_t* u8x8, uint8_t msg, uint8_t arg_int, void* arg_ptr) {
	...
	switch(msg) {
		...
		case U8X8_MSG_BYTE_INIT:
		{
			if (U8G2_ESP32_HAL_UNDEFINED == u8g2_esp32_hal.sda || U8G2_ESP32_HAL_UNDEFINED == u8g2_esp32_hal.scl) {
				break;
			}
			...
			ESP_LOGI(TAG, "clk_speed %u", (uint)handle_i2c.cfg.master.clk_speed);
		    //conf.master.clk_speed = I2C_MASTER_FREQ_HZ;
			ESP_LOGI(TAG, "i2c_param_config %d", handle_i2c.cfg.mode);
