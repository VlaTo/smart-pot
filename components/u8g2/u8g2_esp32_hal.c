...
uint8_t u8g2_esp32_i2c_byte_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	...
	switch(msg) {
		...
		case U8X8_MSG_BYTE_SEND:
		{
			...
			ESP_ERROR_CHECK(i2c_master_write(handle_i2c, data_ptr, arg_int, ACK_CHECK_EN));
		...
		case U8X8_MSG_BYTE_END_TRANSFER:
		{
			...
			ESP_ERROR_CHECK(i2c_master_cmd_begin(I2C_MASTER_NUM, handle_i2c, pdMS_TO_TICKS(I2C_TIMEOUT)));
...
uint8_t u8g2_esp32_gpio_and_delay_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	...
	// specified then we define those pins as GPIO outputs.
		...
		{
			uint64_t bitmask = 0;
			if (U8G2_ESP32_HAL_UNDEFINED != u8g2_esp32_hal.dc)
			{
				bitmask |= (1ull << u8g2_esp32_hal.dc);
			}
			if (U8G2_ESP32_HAL_UNDEFINED != u8g2_esp32_hal.reset)
			{
				bitmask |= (1ull << u8g2_esp32_hal.reset);
			}
			if (U8G2_ESP32_HAL_UNDEFINED != u8g2_esp32_hal.cs)
			{
	...
	// Set the Software I²C pin to the value passed in through arg_int.
		...
		{
			if (U8G2_ESP32_HAL_UNDEFINED != u8g2_esp32_hal.scl) {
...
//				printf("%c",(arg_int==1?'C':'c'));
	...
	// Set the Software I²C pin to the value passed in through arg_int.
		...
		{
			if (U8G2_ESP32_HAL_UNDEFINED != u8g2_esp32_hal.sda)
			{
